...
# RL01：Agent Failure Taxonomy

## 它解决什么问题

Agent 失败不只有答案错误，还包括决策、工具、状态、通信、权限、运行时和产品承诺失败。

## 核心定义

Failure Taxonomy 按阶段与影响分类：input/context、planning/reasoning、action/tool、state/memory、coordination/protocol、runtime、safety/governance、UX/outcome。
...
~~~text
Architecture View
Run trace -> classify failure -> detector -> containment -> recovery
~~~
...
只统计异常会漏掉“成功状态但结果无用”；加入 outcome 与 trajectory 检查。
...
建立测试矩阵、告警与责任归属。应减少整单失败、重复损失与人工重做。
