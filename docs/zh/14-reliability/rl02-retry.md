...
# RL02：Retry and Fallback

## 它解决什么问题

瞬时故障可重试，但语义错误和副作用盲重试会放大损害。

## 核心定义

Retry 对同一操作再次尝试；Fallback 切换模型、工具、数据源、范围或人工。两者均需分类、预算、退避和幂等。
...
~~~text
Architecture View
Error -> classify transient/permanent -> retry/backoff | fallback | fail
~~~
...
限流、网络抖动和可替代依赖。应减少整单失败、重复损失与人工重做。
