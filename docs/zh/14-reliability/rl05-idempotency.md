...
# RL05：Idempotency and Side-Effect Safety

## 它解决什么问题

重试、恢复和重复消息可能让发送、扣款、删除执行多次。

## 核心定义

Idempotency Key 将逻辑操作绑定唯一结果；Side-Effect Safety 还包括 preview、dedupe、transaction、compensation 和 reconciliation。
...
~~~text
Architecture View
Intent+key -> pending record -> execute -> result record -> reuse
~~~
...
适用于所有外部副作用和 durable recovery。

## 失败与恢复场景

崩溃发生在执行后提交前时，先向目标系统按 key 查询，绝不直接重放。
...
所有外部副作用和 durable recovery。应减少整单失败、重复损失与人工重做。
