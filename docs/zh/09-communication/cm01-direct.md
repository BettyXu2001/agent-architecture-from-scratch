...
description: "Agent 之间需要点对点传递任务或结果，并明确发送者、接收者与关联任务。"
...
Agent 之间需要点对点传递任务或结果，并明确发送者、接收者与关联任务。

## 核心定义

Direct Message 是带 sender、recipient、task_id、type、payload 和 correlation_id 的定向通信；不是复制整个聊天。
...
消息丢失或重复时用 message_id、ack、幂等和超时恢复。
...
调用关系清晰、接收者固定的委派。带来的用户收益必须体现在等待、恢复或一致性上。
