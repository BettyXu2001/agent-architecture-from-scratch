...
status: planned
title: "Shared State"
description: "多个组件需要读写同一任务事实，但自由覆盖会导致竞争和不可恢复状态。"
...
多个组件需要读写同一任务事实，但自由覆盖会导致竞争和不可恢复状态。

## 核心定义

Shared State 是有 schema、版本和原子更新规则的共同事实源；消息通知变化，State 证明当前事实。
...
~~~text
Architecture View
Agents <-> versioned state store -> watchers
~~~
...
适用于计划、任务状态和预算需要统一一致视图。

## 失败与恢复场景

并发写冲突时用 compare-and-swap、事务或单写者，不以最后写入静默覆盖。
...
计划、任务状态和预算需要统一一致视图。带来的用户收益必须体现在等待、恢复或一致性上。
