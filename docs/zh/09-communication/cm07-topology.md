...
description: "通信边决定信息、成本和故障如何传播；全连接并非默认最灵活方案。"
updated_at: 2026-08-24
---

# CM07：Communication Topology

## 它解决什么问题

通信边决定信息、成本和故障如何传播；全连接并非默认最灵活方案。

## 核心定义

Topology 描述允许的通信边：star、tree、chain、ring、mesh、blackboard 或 event bus，并附方向、权限和预算。
...
~~~text
Architecture View
Star | Tree | Mesh | Blackboard/Event Bus
~~~
...
适用于根据控制、隐私、延迟和扩展需求选择最小连接。

## 失败与恢复场景

Mesh 造成消息平方增长与责任不清时，收敛为 Supervisor 或 Blackboard。
...
根据控制、隐私、延迟和扩展需求选择最小连接。带来的用户收益必须体现在等待、恢复或一致性上。
