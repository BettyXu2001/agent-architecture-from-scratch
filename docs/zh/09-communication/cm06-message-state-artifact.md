...
status: planned
title: "Message vs State vs Artifact"
description: "系统常用一条消息同时承担通知、当前事实和大文件，导致恢复和版本混乱。"
updated_at: 2026-08-24
---

# CM06：Message vs State vs Artifact

## 它解决什么问题

系统常用一条消息同时承担通知、当前事实和大文件，导致恢复和版本混乱。

## 核心定义

Message 表达交流意图，State 表达当前可变事实，Artifact 表达可寻址成果；三者用 ID 引用而非互相复制。
...
~~~text
Architecture View
Message(artifact_id) -> State(status/version) -> Artifact Store(content)
~~~
...
消息说完成但 Artifact 不存在时，以 State 转换和 Artifact 原子提交为准。
...
长任务、文件协作和可靠恢复。带来的用户收益必须体现在等待、恢复或一致性上。
