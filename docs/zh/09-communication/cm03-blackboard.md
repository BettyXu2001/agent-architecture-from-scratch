...
status: planned
title: "Blackboard"
description: "多个 Specialist 异步贡献线索，后续角色需要基于公共成果继续，而非互相点名。"
...
多个 Specialist 异步贡献线索，后续角色需要基于公共成果继续，而非互相点名。

## 核心定义

Blackboard 是按类型发布问题、假设、证据和 Artifact 的共享工作区；协调器或触发规则选择下一贡献者。
...
~~~text
Architecture View
Agents <-> Blackboard(entries,claims,artifacts) -> trigger
~~~
...
适用于研究、诊断和多阶段推理中的公共证据积累。

## 失败与恢复场景

无主条目和重复假设堆积时用 schema、owner、TTL、去重与验收状态治理。
...
研究、诊断和多阶段推理中的公共证据积累。带来的用户收益必须体现在等待、恢复或一致性上。
