...
status: planned
title: "Private State and Context Packets"
description: "Agent 需要保留局部草稿和观察，同时向外只暴露协作必需结果。"
updated_at: 2026-08-24
---

# CM05：Private State and Context Packets

## 它解决什么问题

Agent 需要保留局部草稿和观察，同时向外只暴露协作必需结果。

## 核心定义

Private State 归单一角色所有；Context Packet 是经裁剪的跨边界输入；Result Packet 是经清洗的输出。
...
~~~text
Architecture View
Private A -> packet -> Private B -> result packet -> Shared
~~~
...
适用于隐私隔离、角色专注和最小上下文传递。

## 失败与恢复场景

把私有链路日志直接共享造成泄漏时，输出网关按字段与来源过滤。
...
隐私隔离、角色专注和最小上下文传递。带来的用户收益必须体现在等待、恢复或一致性上。
