...
description: "多个 Supervisor 形成树状委派，必须定义父子责任、结果回传和全局预算。"
updated_at: 2026-08-24
---

# HY02：Supervisor Tree

## 它解决什么问题

多个 Supervisor 形成树状委派，必须定义父子责任、结果回传和全局预算。
...
~~~text
Architecture View
Root -> branch supervisors -> leaf workers; results flow upward
~~~
...
同一任务被两个分支重复接收时用 task_id 与 ownership 阻断。
...
树结构适合稳定领域分区和自顶向下预算。，并以成功率、并行收益或上下文下降证明价值。
