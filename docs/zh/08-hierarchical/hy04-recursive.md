...
description: "任务分解深度在运行前未知时，Agent 可能递归创建子任务，但必须保证收敛。"
updated_at: 2026-08-24
---

# HY04：Recursive Delegation

## 它解决什么问题

任务分解深度在运行前未知时，Agent 可能递归创建子任务，但必须保证收敛。
...
~~~text
Architecture View
Task(depth,budget) -> delegate smaller tasks -> base cases -> aggregate
~~~
...
只有任务规模严格减小、存在 base case 且预算可分配时允许递归。

## 失败与恢复场景

子任务与父任务等价导致无限递归时用规模度量、深度和 fingerprint 终止。
...
只有任务规模严格减小、存在 base case 且预算可分配时允许递归。，并以成功率、并行收益或上下文下降证明价值。
