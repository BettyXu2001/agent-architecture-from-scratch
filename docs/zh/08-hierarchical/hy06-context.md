...
status: planned
title: "Hierarchical Context and Result Aggregation"
description: "完整上下文跨层复制会指数膨胀，过度摘要又会丢失用户约束和证据。"
updated_at: 2026-08-24
---

# HY06：Hierarchical Context and Result Aggregation

## 它解决什么问题

完整上下文跨层复制会指数膨胀，过度摘要又会丢失用户约束和证据。
...
~~~text
Architecture View
Goal packet down -> scoped subcontexts -> artifact/result packets up -> synthesis
~~~
...
向下传目标与约束，向上传结果、证据、缺口和成本，不传全部内部对话。

## 失败与恢复场景

多层摘要造成事实漂移时，保留 Artifact 引用和 provenance，由顶层按需回读。
...
向下传目标与约束，向上传结果、证据、缺口和成本，不传全部内部对话。，并以成功率、并行收益或上下文下降证明价值。
