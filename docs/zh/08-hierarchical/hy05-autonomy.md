...
status: planned
title: "Local Autonomy and Global Control"
description: "团队需要根据局部信息灵活执行，同时全局必须控制目标、预算、策略和承诺。"
updated_at: 2026-08-24
---

# HY05：Local Autonomy and Global Control

## 它解决什么问题

团队需要根据局部信息灵活执行，同时全局必须控制目标、预算、策略和承诺。
...
~~~text
Architecture View
Global policy/budget -> local plan/actions -> checkpoints/results upward
~~~
...
上层定义 outcome 与约束，下层选择局部路径并在边界升级。

## 失败与恢复场景

局部优化损害全局目标时，通过共享验收、预算和 escalation 修正。
...
上层定义 outcome 与约束，下层选择局部路径并在边界升级。，并以成功率、并行收益或上下文下降证明价值。
