...
description: "大型目标包含多个内部协作方式不同的团队，单一 Worker 接口无法表达。"
...
大型目标包含多个内部协作方式不同的团队，单一 Worker 接口无法表达。
...
~~~text
Architecture View
Coordinator -> Team(A internal graph) | Team(B internal chat) -> artifacts
~~~
...
把团队作为有契约的黑盒能力，内部可用不同 pattern。

## 失败与恢复场景

团队内部细节泄漏到上层造成耦合时，只回传 contract 结果。
...
把团队作为有契约的黑盒能力，内部可用不同 pattern。，并以成功率、并行收益或上下文下降证明价值。
