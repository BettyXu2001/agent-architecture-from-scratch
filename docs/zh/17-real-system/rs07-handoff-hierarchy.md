---
id: RS07
title: 版本 7：Handoff 与层级团队
lang: zh
status: complete
module: 17-real-system
prerequisites: [Part 0, Part 6, Part 11]
concepts: [handoff,hierarchical-team,ownership]
example: enterprise-research
last_reviewed: 2026-08-25
---

# 版本 7：Handoff 与层级团队

> 在对话所有权和组织规模扩大时建立清晰的控制转移。

## 它解决什么问题

有些任务不是“主 Agent 调专家再回答”，而是用户需要与合规或行业专家持续澄清；同时专家数量增加后，单一 Supervisor 成为瓶颈。Handoff 解决会话所有权转移，层级团队解决管理跨度。

## 核心定义与工作原理

Triage Agent 可把会话 Handoff 给行业 Team Lead 或 Compliance Agent；Team Lead 再管理所属 Specialist。每次 Handoff 都携带原因、目标、允许传递的上下文和返回条件，并更新唯一 current_owner。层级深度、转交次数和总预算都有上限。

### 架构视图（Architecture View）

分析时固定追问：谁拥有控制权，状态存在哪里，模型能决定什么，失败从哪里恢复，人工在哪个边界介入，运行证据如何被观察。

### 产品视图（Product View）

界面必须清楚说明当前由谁处理、为何转交、上下文带走了什么以及如何返回。观察 Handoff 接受率、转交后完成率、重复提问率、循环率、用户困惑反馈和队列等待时长。

## 简单实践

画出用户→Triage→Research Lead→两个 Specialist 的所有权图。模拟用户在研究中提出合规问题：是临时调用合规工具，还是 Handoff？根据是否需要持续对话来决策，并写回主流程规则。

实践只要求架构图、状态或消息契约、失败路径和产品指标，不要求安装依赖或运行代码。

## 正常场景

任务按约定入口进入，编排层保留业务状态，各节点或 Agent 产出结构化结果，质量门控制下一阶段，用户获得可理解的进度与最终产物。

## 失败与恢复场景

模型超时、工具失败或子任务质量不足时，区分可重试、可降级和必须人工接管的错误。恢复应依赖业务状态或检查点，而不是把整段任务盲目重跑。

## 什么时候使用

- 核心抽象与任务形态一致。
- 团队能运维它的状态、观测和升级路径。
- 能满足审批、权限、恢复与成本约束。

## 什么时候不要使用

用户始终只需统一答复时，用 Supervisor 调工具更简单。不要把组织架构照搬成 Agent 树，不要发生双重所有权、循环转交或无提示换人格。

## Trade-offs

| 获得 | 代价 |
| --- | --- |
| 更清晰地表达目标架构 | 增加建模和治理成本 |
| 复用运行与生态能力 | 接受版本、依赖和迁移成本 |
| 提升自主性或协作能力 | 增加时延、成本与故障面 |

## 产品视角

- **用户与业务问题**：解决哪个可量化瓶颈？
- **产品价值**：提升成功率、速度、可控性还是覆盖范围？
- **用户体验**：进度、等待、切换和失败是否可理解？
- **自主性边界**：哪些判断交给模型，哪些由代码或人决定？
- **数据与权限**：状态、敏感数据和工具权限如何隔离？
- **失败与降级**：能否重试、续跑、切回简单流程或人工处理？
- **产品指标**：完成率、P95 时延、单任务成本、人工介入率、恢复成功率。

## 框架中的对应实现

Handoff、current_owner、team lead、hierarchical supervisor、return contract、context filter。

## Related Patterns

Workflow、Graph Orchestration、Supervisor、Handoff、Context Isolation、HITL、Durable Execution、Evals。

## 检查清单

- [ ] 产品约束先于实现选择
- [ ] 区分代码控制与模型控制
- [ ] 明确状态事实源、预算和恢复点
- [ ] 覆盖权限、审计和可观测性
- [ ] 记录组织成本与退出方案

## 延伸练习与参考资料

用同一产品场景改画另一种架构，比较运行语义而非 API 长短；继续阅读对应框架的官方架构、持久化、HITL 与观测文档。