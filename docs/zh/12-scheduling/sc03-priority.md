...
# SC03：Scheduler and Priority

## 它解决什么问题

多个任务争用有限模型、工具和人力，需要可解释顺序与公平性。

## 核心定义

Scheduler 从 ready queue 按 priority、deadline、age、cost 和 tenant quota 选择；优先级是业务政策，不由模型任意声明。
...
~~~text
Architecture View
Ready queue -> policy scheduler -> leases -> workers
~~~
...
适用于后台任务、SLA 和多租户资源分配。

## 失败与恢复场景

高优先级流量饿死普通任务时用 aging、配额与保留容量。
...
后台任务、SLA 和多租户资源分配。带来的价值应表现为更短等待、更高 SLA 或更少资源浪费。
