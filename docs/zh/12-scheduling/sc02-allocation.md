...
# SC02：Task Allocation and Capability Matching

## 它解决什么问题

把任务分给第一个可用 Agent 会造成能力、权限、数据域或成本不匹配。

## 核心定义

Capability Matching 将任务要求与 Agent 的 skills、interfaces、trust、data access、capacity、cost 和 SLA 比较。
...
~~~text
Architecture View
Task requirements x Agent cards -> feasible set -> scorer -> assignment
~~~
...
适用于动态 Specialist 池和跨组织 Agent。

## 失败与恢复场景

所有候选不满足硬约束时返回 unassigned，不让低分 Agent 猜着做。
...
动态 Specialist 池和跨组织 Agent。带来的价值应表现为更短等待、更高 SLA 或更少资源浪费。
