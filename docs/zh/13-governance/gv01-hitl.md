...
# GV01：Human-in-the-Loop

## 它解决什么问题

Agent 的不确定性、风险或价值判断需要人参与，但每一步都审批会消灭自动化收益。

## 核心定义

HITL 在选定决策点让人提供输入、判断或纠正；触发来自风险、低置信度、异常或用户偏好。
...
~~~text
Architecture View
Agent -> trigger -> waiting_human -> decision -> resume
~~~
...
等待期间任务超时或上下文过期时保存 Checkpoint，恢复前重新验证。
...
关键判断需要人类语境或责任。使高价值自动化能够在可接受风险下上线。
