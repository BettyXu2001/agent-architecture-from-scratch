...
# GV04：Mixed-Initiative Collaboration

## 它解决什么问题

纯 Agent 自主或纯人工命令都不适合共同编辑、研究和决策。

## 核心定义

Mixed-Initiative 允许用户与 Agent 轮流提出计划、修改范围和执行，但每一时刻的控制权与待决问题显式。
...
~~~text
Architecture View
Shared plan <- user proposals / agent proposals -> accepted actions
~~~
...
适用于用户希望共同探索、随时纠偏的创作和分析。

## 失败与恢复场景

Agent 在用户编辑计划后继续旧分支时，用 plan version 使旧动作失效。
...
用户希望共同探索、随时纠偏的创作和分析。使高价值自动化能够在可接受风险下上线。
