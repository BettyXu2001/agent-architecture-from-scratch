...
# GV06：Permission and Action Boundaries

## 它解决什么问题

Agent 的能力清单不应等于用户权限，连接凭据也不应自动授予所有动作。

## 核心定义

Permission Boundary 以主体、资源、动作、条件和目的执行最小权限；Action Boundary 按 read/draft/execute 与可逆性分级。
...
~~~text
Architecture View
Identity + policy + action proposal -> authorize/step-up/deny
~~~
...
跨用户缓存或 token passthrough 导致越权时，以服务端鉴权、受众绑定和租户隔离阻断。
...
所有外部数据与副作用工具。使高价值自动化能够在可接受风险下上线。
