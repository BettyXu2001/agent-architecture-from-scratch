...
# EV07：Online Evaluation and Human Feedback

## 它解决什么问题

离线集无法覆盖真实分布、供应依赖和用户期望，需要安全地从线上学习。

## 核心定义

Online Eval 结合行为指标、抽样评分、用户纠正、人工审阅、A/B 与 shadow；反馈需区分满意度、事实纠正和偏好。
...
~~~text
Architecture View
Production traces -> privacy sampling -> metrics/review -> dataset/actions
~~~
...
上线监控、漂移检测和产品迭代。让团队能证明架构升级是否值得并阻止回归上线。
