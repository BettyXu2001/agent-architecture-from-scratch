...
# EV02：Task-level Evaluation

## 它解决什么问题

最终答案看似不错不代表用户目标完成；需按任务定义输入、环境、成功和副作用。

## 核心定义

Task Eval 对完整场景评分：outcome correctness、constraint satisfaction、artifact validity、side effects、time 和 cost。
...
~~~text
Architecture View
Scenario -> run in controlled environment -> outcome grader -> score
~~~
...
只用 LLM judge 会把流畅当成功；加入确定性状态和 Artifact 验证。
...
产品级成功率与架构对比。让团队能证明架构升级是否值得并阻止回归上线。
