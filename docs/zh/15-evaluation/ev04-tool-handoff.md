...
# EV04：Tool and Handoff Evaluation

## 它解决什么问题

工具与转移是高影响决策，需要独立测量选择、参数、权限和结果使用。

## 核心定义

Tool Eval 分 choice/arguments/result-use/side-effect；Handoff Eval 分 target/timing/context/ownership/return。
...
~~~text
Architecture View
Decision spans -> specialized graders -> error taxonomy
~~~
...
只检查调用成功会漏掉调用错工具；评价意图适配和后续是否正确使用 Observation。
...
工具密集和多角色产品。让团队能证明架构升级是否值得并阻止回归上线。
