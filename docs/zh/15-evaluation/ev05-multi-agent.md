...
# EV05：Multi-Agent Evaluation

## 它解决什么问题

最终输出无法说明 Specialist 是否有贡献、协调是否浪费或错误如何级联。

## 核心定义

Multi-Agent Eval 同时评价局部角色表现、通信、委派、合成、全局预算、冲突和故障隔离，并做贡献归因。
...
~~~text
Architecture View
Delegation graph + result packets -> local/global graders -> attribution
~~~
...
适用于Supervisor、Handoff、Hierarchy 和 Group Chat。

## 失败与恢复场景

Agent 数增加导致调用增加但消融后质量不变，说明角色无贡献，应删除。
...
Supervisor、Handoff、Hierarchy 和 Group Chat。让团队能证明架构升级是否值得并阻止回归上线。
