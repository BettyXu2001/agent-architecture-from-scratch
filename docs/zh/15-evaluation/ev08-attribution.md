...
# EV08：Cost、Latency and Quality Attribution

## 它解决什么问题

总平均成本无法解释哪个角色、步骤或重试值得，质量提升也可能被失败任务成本掩盖。

## 核心定义

Attribution 将 token、模型、工具、等待、失败与质量映射到 Trace/Span、Agent、Pattern、场景和成功任务。
...
~~~text
Architecture View
Usage spans + outcome scores -> per-success attribution -> trade-off frontier
~~~
...
只算成功调用忽略失败与重试时，采用单成功任务全摊成本和 P95。
...
架构选型、容量和优化。让团队能证明架构升级是否值得并阻止回归上线。
