...
section: memory
status: planned
title: "Memory Retrieval、Write Policy and Forgetting"
description: "记忆价值取决于何时读、什么能写、何时忘，而不取决于存储量。"
updated_at: 2026-08-24
---

# MM06：Memory Retrieval、Write Policy and Forgetting

## 它解决什么问题

记忆价值取决于何时读、什么能写、何时忘，而不取决于存储量。

## 核心定义

Read Policy 按任务与权限选择；Write Policy 决定候选、验证和归属；Forgetting 删除、降权或归档过期内容。 Memory 与 Context 的边界是：Memory 是可被保存和检索的信息，Context 是某次调用实际可见的视图。
...
~~~text
Architecture View
Event -> write gate -> store -> retrieval gate -> context; TTL/delete
~~~
...
让记忆可靠、可控且成本稳定。检索结果保留来源，并只在当前任务和主体权限匹配时进入 Context。

## 失败与恢复场景

模型自报内容直接写入会污染长期记忆；先分事实、偏好、推断并采用不同确认策略。
