...
description: "系统需要复用成功做法，但不能让一次偶然轨迹自动变成高权限流程。"
updated_at: 2026-08-24
---

# MM04：Procedural / Skill Memory

## 它解决什么问题

系统需要复用成功做法，但不能让一次偶然轨迹自动变成高权限流程。

## 核心定义

Procedural Memory 保存经过验证的步骤、策略或 Skill；读取影响如何做，区别于 Semantic 的知道什么。 Memory 与 Context 的边界是：Memory 是可被保存和检索的信息，Context 是某次调用实际可见的视图。
...
~~~text
Architecture View
Episodes -> candidate procedure -> eval/approval -> versioned skill
~~~
...
把重复成功流程产品化。检索结果保留来源，并只在当前任务和主体权限匹配时进入 Context。

## 失败与恢复场景

未经评估自动学习会固化错误；发布前需要测试、权限审查和版本回滚。
