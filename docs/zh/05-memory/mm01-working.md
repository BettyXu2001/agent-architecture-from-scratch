...
description: "当前任务的计划、最近观察与临时变量需要跨步骤保留，但不应永久记住。"
...
当前任务的计划、最近观察与临时变量需要跨步骤保留，但不应永久记住。

## 核心定义

Working Memory 是任务生命周期内可更新的结构化 State；任务结束后默认归档或删除。 Memory 与 Context 的边界是：Memory 是可被保存和检索的信息，Context 是某次调用实际可见的视图。
...
~~~text
Architecture View
Goal -> working state -> next context; complete -> artifact/delete
~~~
...
短期连贯与恢复。检索结果保留来源，并只在当前任务和主体权限匹配时进入 Context。

## 失败与恢复场景

把聊天历史当状态会导致重复副作用；应保存显式字段和版本。
