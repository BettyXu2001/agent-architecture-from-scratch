...
description: "跨任务稳定事实与偏好需要被检索，但来源和有效期必须清楚。"
updated_at: 2026-08-24
---

# MM03：Semantic Memory

## 它解决什么问题

跨任务稳定事实与偏好需要被检索，但来源和有效期必须清楚。

## 核心定义

Semantic Memory 保存实体事实、用户偏好或领域知识，并携带 provenance、confidence 与 validity。 Memory 与 Context 的边界是：Memory 是可被保存和检索的信息，Context 是某次调用实际可见的视图。
...
~~~text
Architecture View
Evidence -> fact extraction -> validate -> semantic store -> retrieve
~~~
...
个性化和持续知识。检索结果保留来源，并只在当前任务和主体权限匹配时进入 Context。

## 失败与恢复场景

错误事实被反复放大时，支持冲突检测、纠正、版本和遗忘。
