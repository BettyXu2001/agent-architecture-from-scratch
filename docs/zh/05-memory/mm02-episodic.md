...
description: "Agent 需要利用过去任务经验，却不能把每次对话都当永久事实。"
updated_at: 2026-08-24
---

# MM02：Episodic Memory

## 它解决什么问题

Agent 需要利用过去任务经验，却不能把每次对话都当永久事实。

## 核心定义

Episodic Memory 保存带时间、主体、结果和反馈的事件记录，检索相似经历而非直接当规则。 Memory 与 Context 的边界是：Memory 是可被保存和检索的信息，Context 是某次调用实际可见的视图。
...
~~~text
Architecture View
Run -> episode(summary,outcome,feedback) -> retrieve by task
~~~
...
从历史成功与失败中复用经验。检索结果保留来源，并只在当前任务和主体权限匹配时进入 Context。

## 失败与恢复场景

过期事件误导当前任务时，用时间衰减、结果质量和用户删除过滤。
