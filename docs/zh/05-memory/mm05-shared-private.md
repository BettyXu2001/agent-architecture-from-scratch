...
description: "多 Agent 需要共享成果，但全局共享会泄漏数据并造成角色互相污染。"
updated_at: 2026-08-24
---

# MM05：Shared vs Private Memory

## 它解决什么问题

多 Agent 需要共享成果，但全局共享会泄漏数据并造成角色互相污染。

## 核心定义

Private Memory 属于特定主体或角色；Shared Memory 只保存协作必需、经过清洗的事实与 Artifact。 Memory 与 Context 的边界是：Memory 是可被保存和检索的信息，Context 是某次调用实际可见的视图。
...
~~~text
Architecture View
Private A -> sanitized artifact -> shared workspace <- Private B
~~~
...
兼顾专业隔离与协作。检索结果保留来源，并只在当前任务和主体权限匹配时进入 Context。

## 失败与恢复场景

共享区成为无主垃圾场时，用 owner、schema、TTL 和写入责任治理。
