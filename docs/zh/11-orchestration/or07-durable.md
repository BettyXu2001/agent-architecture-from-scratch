...
# OR07：Durable Execution、Checkpoint and Resume

## 它解决什么问题

长任务跨进程重启、用户等待与外部超时，内存循环无法保证恰好继续。

## 核心定义

Durable Execution 持久化 Workflow/Agent 的逻辑状态和操作结果，使运行可重放或恢复；Checkpoint 是一致恢复点。
...
~~~text
Architecture View
Command -> persist intent -> execute/idempotent -> persist result -> next
~~~
...
适用于后台研究、审批、异步 Agent 和多小时任务。

## 失败与恢复场景

进程在副作用后、结果提交前崩溃时，用幂等键查询/重放，不能直接重复执行。
...
后台研究、审批、异步 Agent 和多小时任务。应带来更高恢复率、更短等待或更少人工协调。
