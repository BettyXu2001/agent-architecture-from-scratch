...
具有明确依赖的任务需要并行和拓扑调度，同时禁止循环。

## 核心定义

DAG 以节点表示任务、边表示单向依赖；只有所有必要上游完成后节点才 ready。
...
~~~text
Architecture View
A -> [B|C] -> D; topological order; no cycles
~~~
...
适用于批处理、数据分析和稳定多阶段 Workflow。

## 失败与恢复场景

依赖环在运行前检测；上游失败向下游传播 blocked。
...
批处理、数据分析和稳定多阶段 Workflow。应带来更高恢复率、更短等待或更少人工协调。
