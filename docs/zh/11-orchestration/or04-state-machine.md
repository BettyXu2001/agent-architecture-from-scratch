...
产品必须明确任务处于什么状态、允许哪些操作以及如何从失败恢复。

## 核心定义

State Machine 用有限状态、事件、guard 和 transition 表达生命周期；非法转换被拒绝。
...
适用于审批、Handoff、远程任务和用户可见状态。

## 失败与恢复场景

重复完成、取消后继续执行等竞态用版本与终态不可逆规则处理。
...
审批、Handoff、远程任务和用户可见状态。应带来更高恢复率、更短等待或更少人工协调。
