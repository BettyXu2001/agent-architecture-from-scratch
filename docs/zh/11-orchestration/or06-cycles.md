...
# OR06：GraphFlow and Cyclic Workflows

## 它解决什么问题

评审、重规划和对话需要受控回边，但普通 Workflow 的循环常隐藏在代码中。

## 核心定义

Cyclic Graph 将循环显式为边，并为回边定义条件、状态变化、最大次数、deadline 与 no-progress。
...
~~~text
Architecture View
draft -> evaluate -> revise -> evaluate -> END; bounded cycle
~~~
...
适用于Evaluator、Replanner 和迭代协作。

## 失败与恢复场景

每轮只改措辞没有状态进展时，哈希/分数无改进触发最佳候选返回。
...
Evaluator、Replanner 和迭代协作。应带来更高恢复率、更短等待或更少人工协调。
