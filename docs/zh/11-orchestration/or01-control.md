...
# OR01：Code-driven vs LLM-driven Orchestration

## 它解决什么问题

控制路径需要在可预测与适应性之间分配，而不是全交给代码或模型。

## 核心定义

Code-driven 由程序选择节点和边；LLM-driven 由模型基于 Context 选择；Hybrid 用代码包围有限模型决策区。
...
~~~text
Architecture View
Request -> code gates -> LLM decision zone -> code policy -> next node
~~~
...
根据路径可枚举性与风险分配控制权。应带来更高恢复率、更短等待或更少人工协调。
